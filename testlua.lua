...
elseif id == 4442272183 then Second_Sea = true; elseif id == 7449423635 then Third_Sea = true; else game:Shutdown() end;
--------------------------------------------------------------------------------------------------------------------------------------------
--Flag Player
function AntiBan()
    ...
    -- Fix lỗi nil cho Character
    local charDescendants = char:GetDescendants() or {}
    if type(charDescendants) == "table" then
    ...
    -- Khắc phục triệt để lỗi báo đỏ "nil" ở PlayerScripts
    local playerScripts = player:WaitForChild("PlayerScripts", 5)
    if playerScripts then
        local scriptDescendants = playerScripts:GetDescendants() or {}
        if type(scriptDescendants) == "table" then
            for i,v in pairs(scriptDescendants) do
                if v:IsA("LocalScript") then
...
local listfastattack = {'Normal Attack','Fast Attack','Super Fast Attack'}
    ...
    local ToggleLevel = Tabs.Main:AddToggle("ToggleLevel", {Title = "Tự Cày Cấp", Default = false })
    ...
    spawn(function()
        ...
        local ToggleCandy = Tabs.Main:AddToggle("ToggleCandy", {Title = "Tự Fram Kẹo ( Chỉ Khi Max Lever )", Default = false })
...
--------------------------------------------------------------------------------------------------------------------------------------------
    ...
    local ToggleMobAura = Tabs.Main:AddToggle("ToggleMobAura", {Title = "Đánh Quái Gần | Mob Aura", Default = false })
    ...
    local ToggleCastleRaid = Tabs.Main:AddToggle("ToggleCastleRaid", {Title = "Đánh Hải Tặc | Pirates Castle", Default = false })
...
if Third_Sea then
local ToggleChest = Tabs.Main:AddToggle("ToggleChest", {Title = "Nhặt Rương Dừng Khi Có Cup [Tp]", Default = false })
...
local ToggleChestTween = Tabs.Main:AddToggle("ToggleChestTween", {Title = "Nhặt Rương [Tween]", Default = false })
...
local ToggleCake = Tabs.Main:AddToggle("ToggleCake", {Title = "Đánh Cake Prince", Default = false })
...
spawn(function()
    ...
    local ToggleSpawnCake = Tabs.Main:AddToggle("ToggleSpawnCake", {Title = "Triệu Hồi Cake Prince", Default = true })
    ...
    if Second_Sea then
    local ToggleVatChatKiDi = Tabs.Main:AddToggle("ToggleVatChatKiDi", {Title = "Lấy Vật Phẩm Kì Dị", Default = false })
...
local boss = Tabs.Main:AddSection("Boss Farm")
    ...
    spawn(function()
      ...
      local ToggleEspKitsune = Tabs.Main:AddToggle("ToggleEspKitsune", {Title = "Định Vị Kitsune Island", Default = false })
      ...
      local ToggleTPKitsune = Tabs.Main:AddToggle("ToggleTPKitsune", {Title = "Bay Vô Đảo Kitsune", Default = false })
      ...
      local ToggleCollectAzure = Tabs.Main:AddToggle("ToggleCollectAzure", {Title = "Nhặt Lửa", Default = false })
...
if Third_Sea then
    ...
    local ToggleSailBoat = Tabs.Main:AddToggle("ToggleSailBoat", {Title = "Mua Tàu", Default = false })
    ...
    local ToggleShip = Tabs.Main:AddToggle("ToggleShip", {Title = "Làm chơi ko bt là gì", Default = false })
...
while wait() do
    ...
    local ToggleGhostShip = Tabs.Main:AddToggle("ToggleGhostShip", {Title = "Đánh Tàu Ma", Default = false })
...
spawn(function()
    ...
    local ToggleElite = Tabs.Main:AddToggle("ToggleElite", {Title = "Đánh Elite Hunter", Default = false })
...
local ToggleTweenMirageIsland = Tabs.Main:AddToggle("ToggleTweenMirageIsland", {Title = "Bay Vô Đảo Bí Ẩn", Default = false })
...
local ToggleTweenGear = Tabs.Main:AddToggle("ToggleTweenGear", {Title = "Bay Tới Gear", Default = false })
...
local ToggleMirage = Tabs.Main:AddToggle("ToggleMirage", {Title = "Treo Đảo Bí Ẩn", Default = false })
...
if Second_Sea or Third_Sea then
    local ToggleHakiColor = Tabs.Main:AddToggle("ToggleHakiColor", {Title = "Mua Haki Color",Default = false })
...
if Second_Sea then
    local ToggleSwordLengend = Tabs.Main:AddToggle("ToggleSwordLengend", {Title = "Mua Kiếm Legen",Default = false })
...
local ToggleEspFlower = Tabs.Fruit:AddToggle("ToggleEspFlower", {Title = "Định Vị Hoa Sea2", Default = false })
...
spawn(function()
    ...
    local ToggleStart = Tabs.Raid:AddToggle("ToggleStart", {Title = "Vô Đảo Raid", Default = false })
...
local ToggleNextIsland = Tabs.Raid:AddToggle("ToggleNextIsland", {Title = "Qua Đảo Raid Khác", Default = false })
...
local ToggleAwake = Tabs.Raid:AddToggle("ToggleAwake", {Title = "Thức Tỉnh", Default = false })
	...
	Callback = function()
		...
		for k, v in pairs(cac) do
			if ListHuhu[v] and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Buy" .. v, true) == 1 then
				...
				huhu.Image = "rbxassetid://79480507959689"
